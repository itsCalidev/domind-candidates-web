...
/**
 * Único formulario de "Estado de Salud" — compartido entre HealthTab
 * (panel admin, `mode="admin"`) y el paso correspondiente del Wizard de
 * Autollenado (`mode="candidate"`), mismo patrón que PersonalInfoForm/
 * FamilyForm. El candidato nunca ve los paneles de riesgo/resumen de
 ...
 * que no está en la lista de opciones sugeridas — esos valores se
 * agrupan bajo "Otro" con su texto original preservado, en vez de
 * perderse o romper el checkbox.
 */
function splitKnownAndOther(values: string[], knownOptions: readonly string[]): { known: string[]; other: string } {
...
/** Inverso de `splitKnownAndOther`: para el payload, "Otro" nunca se envía tal cual — se reemplaza por lo que se escribió, o se descarta si se dejó vacío. */
...
const EMPTY_HEALTH: CandidateHealth = {
  weight: null,
  height: null,
  usesGlasses: null,
  physicalAspect: null,
  currentHealth: null,
  chronicDiseasesFamily: null,
  chronicDiseasesDetails: null,
  hasPastDiseases: null,
  pastDiseases: null,
  hasSurgeries: null,
  surgeries: null,
  healthcareAccess: [],
...
function buildHealthFormDefaults(health: CandidateHealth | null): HealthFormValues {
  const safeHealth = health ?? EMPTY_HEALTH;
  const healthcareAccess = splitKnownAndOther(safeHealth.healthcareAccess, HEALTHCARE_ACCESS_OPTIONS);
  const alcoholTypes = splitKnownAndOther(safeHealth.alcoholTypes, ALCOHOL_TYPE_OPTIONS);
  return {
    weight: toFormNumber(safeHealth.weight),
    height: toFormNumber(safeHealth.height),
    usesGlasses: toTriState(safeHealth.usesGlasses),
    physicalAspect: safeHealth.physicalAspect ?? '',
    currentHealth: pickOption(CURRENT_HEALTH_OPTIONS, safeHealth.currentHealth),
    chronicDiseasesFamily: toTriState(safeHealth.chronicDiseasesFamily),
    chronicDiseasesDetails: safeHealth.chronicDiseasesDetails ?? '',
    pastDiseasesFlag: toTriState(safeHealth.hasPastDiseases),
    pastDiseases: safeHealth.pastDiseases ?? '',
    surgeriesFlag: toTriState(safeHealth.hasSurgeries),
    surgeries: safeHealth.surgeries ?? '',
    healthcareAccess: healthcareAccess.known,
    healthcareAccessOther: healthcareAccess.other,
    alcoholFrequency: pickOption(ALCOHOL_FREQUENCY_OPTIONS, safeHealth.alcoholFrequency),
    alcoholTypes: alcoholTypes.known,
    alcoholTypesOther: alcoholTypes.other,
    smokes: toTriState(safeHealth.smokes),
    cigarettesPerDay: toFormNumber(safeHealth.cigarettesPerDay),
    smokingExpensePerWeek: toFormNumber(safeHealth.smokingExpensePerWeek),
    usedDrugs: toTriState(safeHealth.usedDrugs),
    drugsDetails: safeHealth.drugsDetails ?? '',
    dietQuality: pickOption(DIET_QUALITY_OPTIONS, safeHealth.dietQuality),
    physicalActivity: pickOption(PHYSICAL_ACTIVITY_OPTIONS, safeHealth.physicalActivity),
    sedentaryHours: toFormNumber(safeHealth.sedentaryHours),
    screenTimeHours: toFormNumber(safeHealth.screenTimeHours),
  };
}
 ...
 * (`'yes' | 'no' | ''`) a un booleano real — `''` (todavía sin
 * responder) se traduce a `undefined` para no enviar nada en ese campo.
 */
 ...
 * backend acepta un PATCH parcial, así que se manda únicamente lo que se
 * realmente tocó, mismo criterio que `updatePersonalInfo`.
 */
...
function HabitChip({ label, severity, icon }: { label: string; severity: HabitSeverity | 'info'; icon?: ReactElement }) {
  return (
    <Chip
      icon={icon}
      label={label}
...
export function HealthForm(props: HealthFormProps) {
  ...
  const {
    ...
    defaultValues: buildHealthFormDefaults(props.health),
  });
  ...
  function handleStartEditing() {
    reset(buildHealthFormDefaults(props.health));
    setIsEditing(true);
  }
  ...
  const smokingStatus: 'pending' | 'no' | 'yes' = health.smokes === null ? 'pending' : health.smokes ? 'yes' : 'no';
            ...
            {health.healthcareAccess.length === 0 ? (
              ...
              <Stack direction="row" flexWrap="wrap" gap={0.75}>
                {health.healthcareAccess.map((service) => (
                  <Chip key={service} icon={<HealthAndSafetyOutlinedIcon />} label={service} size="small" variant="outlined" />
                ))}
              </Stack>
            )}
          </SummaryCard>
              ...
              {hasNoAlcoholData ? (
                ...
                <Stack direction="row" flexWrap="wrap" gap={0.75}>
                  {!isEmptyMedicalText(health.alcoholFrequency) && (
                    <HabitChip label={health.alcoholFrequency!} severity={alcoholSeverity} icon={<WineBarOutlinedIcon />} />
                  )}
              ...
              {smokingStatus === 'pending' ? (
                ...
                <Stack direction="row" flexWrap="wrap" gap={0.75}>
                  <Chip
                    ...
                    label={health.cigarettesPerDay !== null ? `${health.cigarettesPerDay} cigarros/día` : 'Cantidad no especificada'}
                    color="warning"
                    size="small"
                  />
                  {health.smokingExpensePerWeek !== null && (
                    <Chip icon={<SmokingRoomsOutlinedIcon />} label={`$${health.smokingExpensePerWeek}/semana`} variant="outlined" size="small" />
                  )}
                </Stack>
              )}
            </Box>
